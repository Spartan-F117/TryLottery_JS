    ...
    const BuyTicketsHandler = async () =>{
        try{
            ...
            for (let i=0; i < arraynumbers.length; i++){
                if(!(/^\d+$/.test(arraynumbers[i]))){
                    setError("just number separated values!")
                    return
                }
            }

            if(!(/^\d+$/.test(TicketPowerball))){
                setError("Powerball must contains only 1 number")
                return
            }
